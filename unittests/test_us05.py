...
class TestUS05(unittest.TestCase):
    ...
    def setUp(self):
        individuals, families = parse("../testfiles/US05_test.ged")
    ...
    def test_marriages_good(self):
        ...
        result, output = Checks.marriage_before_death(self.individuals, self.families)
        self.assertEqual(result, True)
        self.assertEqual(
            output, "All individuals were married before death.\n")
        # put things back
        self.individuals[6].death = "17 JAN 1979"

    def test_marriages_bad(self):
        result, output = Checks.marriage_before_death(self.individuals, self.families)
        self.assertEqual(result, False)
        self.assertEqual(output, "Error: " + self.individuals[6].ID + " was married in family " + self.families[1].ID + " after death.\n")

    def test_empty_families(self):
        result, output = Checks.marriage_before_death(self.individuals, [])
        self.assertEqual(result, True)
        self.assertEqual(
            output, "All individuals were married before death.\n")
